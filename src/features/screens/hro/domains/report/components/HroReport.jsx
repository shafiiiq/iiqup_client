...
import {
  ...
  buildTimestamp,
  buildPdf,
} from '../helper/hro.report.helper';
import './HroReport.css';
...
function HroReport() {
  ...
  useEffect(() => {
    const initializeDeviceInfo = async () => {
      try {
        ...
        if (!user._id) {
          console.warn('[HroReport] User ID not found in localStorage');
        ...
        console.error('[HroReport] Failed to initialize device info:', err);
        setGlobalActivation({ isActivated: false, isTrusted: false, checked: true });
      }
    };
  ...
  const loadSignature = async (signType, info, flags, authTitle = hireOrderAuthSignatoryTitle) => {
    ...
    try {
      ...
      console.error(`[HroReport] loadSignature(${signType}) error:`, err);
  ...
  const handleSignButtonClick = async () => {
    ...
    try {
      ...
      console.error('[HroReport] handleSignButtonClick error:', err);
  ...
  const handleConfirmSign = async (override = false) => {
     ...
     try {
       ...
       console.error('[HroReport] handleConfirmSign error:', err);
  ...
  const handleSendEmail = async (extraFiles = []) => {
    ...
    try {
      ...
      console.error('[HroReport] handleSendEmail error:', err);
  ...
  const handleActivation = async () => {
    ...
    try {
      ...
      console.error('[HroReport] Activation error:', err);
  ...
  const checkAllSignTypeTrust = async (info) => {
    ...
    try {
      ...
      console.error('[HroReport] checkAllSignTypeTrust error:', err);
  ...
  const fetchHireOrder = async () => {
    ...
    try {
      ...
      console.error('[HroReport] fetchHireOrder error:', err);
  ...
  const sendToApprove = async () => {
    ...
    await withControlsHidden(async () => {
      try {
        ...
        console.error('[HroReport] sendToApprove error:', err);
  ...
  const handleDownloadPdf = async () => {
    ...
    await withControlsHidden(async () => {
      try {
        ...
        console.error('[HroReport] handleDownloadPdf error:', err);
