...
import { buildPdf, formatCurrency, signatoryRole, buildTimestamp } from '../helper/quotation.report.helper';
...
function QuotationDoc() {
  ...
  const loadSignature = async (signType, info, flags, authTitle = hireOrderAuthSignatoryTitle) => {
    ...
    try {
      ...
      if (!s3Data?.dataUrl) throw new Error('Failed to get signature URL');
  ...
  const fetchHireOrder = async () => {
    ...
    try {
      ...
      const response = await getHireOrderByRef(refNo);
