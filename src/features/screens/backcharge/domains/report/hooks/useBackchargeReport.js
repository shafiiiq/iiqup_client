...
const DEFAULT_FORM_DATA = {
  ...
  siteLocation: '',
  workDate: '',
  scopeOfWork: { line1: '', line2: '', combinedText: '' },
  workshopComments: { line1: '', line2: '', line3: '', line4: '', combinedText: '' },
...
export const useBackchargeReport = () => {
  ...
  const fetchBackchargeData = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        setFormData({ ...data.data, tableRows: normaliseTableRows(data.data.tableRows, TABLE_ROW_COUNT) });
        setDocumentExists(true);
        setDocumentId(data.data._id);
        setSignatureFlags(data.data.signatureFlags || {});
        setSignatureStates(data.data.signatureStates || {});
        setSupplierMail(data.data.supplierEmail);
      }
    } catch (error) {
  ...
  const checkActivationStatus = async (info) => {
    try {
      const response = await verifyDeviceTrust(info);
  ...
  const handleDownloadPdf = async () => {
    try {
      hideControls();
      await buildPdf(componentRef, getFileName());
    } catch (error) {
  ...
  const handleActivation = async () => {
    ...
    try {
      ...
      const response = await activateSignature(deviceInfo, activationKey);
      if (response.success) {
  ...
  const handleConfirmSign = async (override = false) => {
    try {
      ...
      const response = await signBackcharge({
        backchargeId: documentId,
        signatureFlags,
        override,
        deviceInfo,
