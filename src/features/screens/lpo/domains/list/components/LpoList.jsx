...
import { deleteLpo, fetchLpoList, fetchPendingSignatures } from '../api/lpo.list.api';
...
function LpoList({ isAll, isEquip, isStock, isForAllEquip }) {
  ...
  const fetchLpos = async () => {
    setIsLoading(true);
    try {
      const data = await fetchLpoList({ isEquip, isStock, isForAllEquip, regNo });
      setLpos(data.data);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteLpo(selectedLpo.lpoRef);
