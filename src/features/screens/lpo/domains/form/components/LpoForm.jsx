// ─────────────────────────────────────────────────────────────────────────────
// LpoForm.jsx — LPO (Local Purchase Order) creation, editing, and amendment form.
...
import {
  ...
  getPreSignedUrl,
} from '../api/lpo.form.api';
import {
  generateLpoRef,
  formatDate,
  formatCurrency,
  filterEditableTerms,
} from '../helper/lpo.form.helper';
...
// ─────────────────────────────────────────────────────────────────────────────
// LpoForm — Main Component
// ─────────────────────────────────────────────────────────────────────────────

/**
 * LpoForm — Multi-mode LPO form component.
 *
 ...
 */
function LpoForm({ isStock, isAllEquip, edit, amendment, amendmentEdit }) {
  ...
  const handleRouteSpecificLogic = async () => {
    ...
    try {
      ...
      console.error('[LpoForm] handleRouteSpecificLogic error:', err);
    }
  };
  ...
  const fetchLatestLpoNumber = async () => {
    try {
      ...
      console.error('[LpoForm] fetchLatestLpoNumber error:', err);
  ...
  const fetchLpoForEdit = async () => {
    ...
    try {
      ...
      console.error('[LpoForm] fetchLpoForEdit error:', err);
  ...
  const fetchLpoForAmendmentEdit = async () => {
    ...
    try {
      ...
      console.error('[LpoForm] fetchLpoForAmendmentEdit error:', err);
      setSaveStatus('Error loading LPO data');
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchEquipments = async (searchTerm = '') => {
    try {
      ...
      console.error('[LpoForm] fetchEquipments error:', err);
    }
  };
  ...
  const fetchCompanies = async () => {
    try {
      ...
      console.error('[LpoForm] fetchCompanies error:', err);
    }
  };
  ...
  const saveLpoData = async () => {
    ...
    try {
      ...
      const result = await createOrUpdateLpo(endpoint, method, payload);
      ...
      if (needsComplaintPost) {
        ...
        const complaintResult = await createComplaintLpo(complaintId, complaintPayload);
      ...
      console.error('[LpoForm] saveLpoData error:', err);
  ...
  const handleQuotationFileChange = async (e) => {
    ...
    try {
      ...
      console.error('[LpoForm] quotation upload error:', err);
  ...
  const loadQuotationPreview = async (quotation) => {
    ...
    try {
      ...
      console.error('[LpoForm] loadQuotationPreview error:', err);
    }
  };
