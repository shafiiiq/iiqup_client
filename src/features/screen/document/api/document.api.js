...
export const fetchDocumentTypes = async () => {
  const response = await apiRequest(`/documents/get-all-documents-types`);
  if (!response.ok) throw new Error('Failed to fetch document types');
  const data = await response.json();
  return data.data?.map((doc) => doc.documentType?.trim()).filter(Boolean) ?? [];
};

export const fetchDocumentsBySource = async ({ sourceType, type, id }) => {
  const response = await apiRequest(`/documents/get-documents/${sourceType || type}/${id}`, 'GET');
...
export const uploadDocument = async (payload) => {
  const response = await apiRequest(`/documents/upload-document`, 'POST', payload);
  const result = await response.json();
  return { response, result };
};

export const getDocumentViewData = async (documentId) => {
  const response = await apiRequest(`/documents/view/${documentId}`);
  if (!response.ok) throw new Error(`Server error: ${response.status}`);
  return response.json();
};

export const getDocumentDownloadData = async (documentId) => {
  const response = await apiRequest(`/documents/download/${documentId}`);
  if (!response.ok) throw new Error(`Server error: ${response.status}`);
  return response.json();
};

export const deleteDocument = async (documentId) => {
  const response = await apiRequest(`/documents/delete/${documentId}`, 'DELETE');
  const result = await response.json();
  return { response, result };
};

export const renameDocument = async (docId, newFileName) => {
  const response = await apiRequest(`/documents/rename-file/${docId}`, 'PUT', { newFileName: newFileName.trim() });
  const result = await response.json();
  return { response, result };
};

export const getSignedUrl = async (filePath, options = {}) => {
  const response = await apiRequest(`/s3/pre-signed-url`, 'POST', {
    key: filePath,
    isLong: false,
    ...options,
  });
...
export const splitPdfDocument = async ({ sourceId, sourceType, documentId, pageNumbers, currentCategory }) => {
  const response = await apiRequest(`/documents/split-pdf`, 'POST', {
    sourceId,
    sourceType,
    documentId,
...
export const mergePdfDocuments = async ({ sourceId, sourceType, documentIds, currentCategory }) => {
  const response = await apiRequest(`/documents/merge-pdfs`, 'POST', {
    sourceId,
    sourceType,
    documentIds,
    category: currentCategory === 'all' ? 'merged' : currentCategory,
