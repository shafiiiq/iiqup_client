...
function BackchargeStatsAnalytics({ series, loading }) {
  if (loading || !series) {
    return <div className="backcharge stats empty"><p>Loading analytics…</p></div>;
  }

  return (
    <div className="backcharge stats analytics">
      <div className="backcharge stats chart-card">
        <div className="backcharge stats chart-header">
          ...
          <p>Compare backcharge volume against total spend per bucket</p>
        </div>
        {series.buckets?.length > 0 ? (
          <ResponsiveContainer width="100%" height={360}>
            <ComposedChart data={series.buckets}>
              ...
              <Line yAxisId="right" type="monotone" dataKey="count" stroke="#7c9cf5" strokeWidth={3} dot name="Backcharges" />
            </ComposedChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="backcharge stats table-card">
        <div className="backcharge stats chart-header">
        ...
        <Table
          columns={[
            ...
            { key: 'count', header: 'Reports', render: (b) => b.count },
            { key: 'total', header: 'Total Cost', render: (b) => formatCurrency(b.total) },
            { key: 'deduction', header: 'Total Deduction', render: (b) => formatCurrency(b.deduction) },
          ]}
          data={series.buckets || []}
          rowKey={(b) => b.label}
          emptyMessage="No backcharge reports for this period"
        />
      </div>
    </div>
  );
...
export default BackchargeStatsAnalytics;