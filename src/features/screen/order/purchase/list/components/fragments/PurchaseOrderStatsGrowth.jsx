...
function PurchaseOrderStatsGrowth({ summary, growth, loading }) {
  if (loading || !growth || !summary) {
    return <div className="purchase order stats empty"><p>Loading growth data…</p></div>;
  }
  ...
  return (
    <div className="purchase order stats growth">
      <div className="purchase order stats chart-card">
        <div className="purchase order stats chart-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h3>Cumulative Spend — {growth.rangeLabel}</h3>
            <p>Running total of purchase order cost across the selected period</p>
          </div>
          <div
            style={{
...
export default PurchaseOrderStatsGrowth;