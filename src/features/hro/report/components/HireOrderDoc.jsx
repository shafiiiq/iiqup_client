...
import {
  ...
  activateSignature,
  uploadHireOrder,
} from '../services/hireOrder.service';
import {
  SIGN_TYPES,
  PAGE_BREAK_THRESHOLDS,
  DEFAULT_COLUMNS,
...
} from '../utils/hireOrderDocHelpers';
...
function HireOrderDoc() {
  ...
  useEffect(() => {
    const initializeDeviceInfo = async () => {
      try {
        ...
        if (!user._id) {
          console.warn('[HireOrderDoc] User ID not found in localStorage');
        ...
        console.error('[HireOrderDoc] Failed to initialize device info:', err);
        setGlobalActivation({ isActivated: false, isTrusted: false, checked: true });
      }
    };
  ...
  const loadSignature = async (signType, info, flags, authTitle = hireOrderAuthSignatoryTitle) => {
    ...
    try {
      ...
      console.error(`[HireOrderDoc] loadSignature(${signType}) error:`, err);
  ...
  const handleSignButtonClick = async () => {
    ...
    try {
      ...
      console.error('[HireOrderDoc] handleSignButtonClick error:', err);
  ...
  const handleConfirmSign = async (override = false) => {
     ...
     try {
       ...
       console.error('[HireOrderDoc] handleConfirmSign error:', err);
  ...
  /** Generates PDF and sends Hire Order to vendor via email. */
  const handleSendEmail = async (extraFiles = []) => {
    ...
    try {
      ...
      console.error('[HireOrderDoc] handleSendEmail error:', err);
  ...
  const handleActivation = async () => {
    ...
    try {
      ...
      console.error('[HireOrderDoc] Activation error:', err);
  ...
  const checkAllSignTypeTrust = async (info) => {
    ...
    try {
      ...
      console.error('[HireOrderDoc] checkAllSignTypeTrust error:', err);
  ...
  const fetchHireOrder = async () => {
    ...
    try {
      ...
      console.error('[HireOrderDoc] fetchHireOrder error:', err);
  ...
  const sendToApprove = async () => {
    ...
    await withControlsHidden(async () => {
      try {
        ...
        console.error('[HireOrderDoc] sendToApprove error:', err);
  ...
  const handleDownloadPdf = async () => {
    ...
    await withControlsHidden(async () => {
      try {
        ...
        console.error('[HireOrderDoc] handleDownloadPdf error:', err);
